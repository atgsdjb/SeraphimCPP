...
main2 (int argc, char **argv)
{
	...
	FILE *yuvFile=fopen("----bridge-close_qcif.yuv","rb");
	assert (pFile);
	assert(yuvFile);
	//* 示例用编码数据.
...
void f(){
	Yuv420 *v = new Yuv420(176,144,2001,"bridge-close_qcif.yuv");
	int i = 10;
	int len = 0;
	...
	FILE *f = fopen("test.yuv","w+");
	for(i;i<1000;i++){
		fwrite(v->getY(),1,l_ysize,f);
